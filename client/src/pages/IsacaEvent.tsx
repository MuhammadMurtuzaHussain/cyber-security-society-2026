...
export default function IsacaEvent() {
  ...
  const handleStart = () => {
    ...
    setNotice("");
    start.mutate(undefined, { onSuccess: (result) => { loadResult(result); if (result.outcome === "event_closed") setNotice("This limited-time event closed at Saturday 17:00 Europe/London."); if (result.outcome === "expired") setNotice("Your one-hour run has expired. The sealed case cannot be restarted."); }, onError: (error) => setNotice(error.message) });
  };
