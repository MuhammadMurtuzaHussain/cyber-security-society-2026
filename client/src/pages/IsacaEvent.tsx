...
export default function IsacaEvent() {
  ...
  const handleStart = () => {
    ...
    setAnswer("");
    start.mutate(undefined, { onSuccess: (result) => { loadResult(result); if (result.restarted) setNotice("Fresh sealed case opened. Restart from Stage 01; this run has a new server-generated evidence set."); if (result.outcome === "event_closed") setNotice("This limited-time event closed at Saturday 17:00 Europe/London."); }, onError: (error) => setNotice(error.message) });
  };
  ...
  useEffect(() => {
    ...
    expire.mutate(undefined, { onSuccess: (result) => { if (result.run) setRun(result.run as Run); setNotice(result.outcome === "event_closed" ? "The event cutoff has passed. This competition run is closed." : "The one-hour server timer has expired. This competition run is closed."); } });
