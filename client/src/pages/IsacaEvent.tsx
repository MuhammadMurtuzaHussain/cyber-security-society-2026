...
function EventStart({ onStart, loading, authenticated }: { onStart: () => void; loading: boolean; authenticated: boolean }) {
  return <section className="relative overflow-hidden rounded-3xl border border-violet-300/20 bg-gradient-to-br from-violet-400/[.11] via-[#0d1422] to-cyan-300/[.06] p-6 sm:p-8"><div className="absolute -right-10 -top-10 h-48 w-48 rounded-full bg-violet-300/[.1] blur-3xl" /><div className="relative grid gap-7 lg:grid-cols-[1fr_.72fr]"><div><p className="font-mono text-[10px] tracking-[.2em] text-violet-200">ISACA EVENT // SEALED COMPETITION</p><h2 className="mt-4 max-w-2xl font-display text-3xl font-bold leading-tight tracking-[-.045em] text-white">The One-Hour <span className="text-violet-300">Control Room.</span></h2><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300">Fifteen stages. No hints. Three attempts per stage. Your run is uniquely seeded and validated by the server, so copying another student’s response will not unlock your case.</p><div className="mt-6 flex flex-wrap gap-2"><span className="rounded-full border border-rose-300/18 bg-rose-300/[.06] px-3 py-1.5 font-mono text-[9px] text-rose-100">1-HOUR HARD LIMIT</span><span className="rounded-full border border-amber-300/18 bg-amber-300/[.06] px-3 py-1.5 font-mono text-[9px] text-amber-100">3 ATTEMPTS / STAGE</span><span className="rounded-full border border-cyan-300/18 bg-cyan-300/[.06] px-3 py-1.5 font-mono text-[9px] text-cyan-100">NO HINTS</span></div><button onClick={onStart} disabled={loading} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-violet-300 px-5 py-3.5 text-xs font-bold text-[#160b25] transition hover:bg-violet-200 disabled:cursor-not-allowed disabled:opacity-60">{loading ? "OPENING SEALED CASE…" : authenticated ? "OPEN SEALED CASE" : "SIGN IN TO COMPETE"} <LockKeyhole className="h-4 w-4" /></button><p className="mt-3 font-mono text-[9px] tracking-[.1em] text-slate-500">STARTING THE CASE BEGINS YOUR SERVER-ENFORCED TIMER.</p></div><div className="rounded-2xl border border-white/[.09] bg-[#060a12]/70 p-5"><p className="font-mono text-[10px] tracking-[.16em] text-cyan-300">CASE DOMAINS</p><div className="mt-5 grid grid-cols-2 gap-2">{["GOVERNANCE", "IT AUDIT", "RISK", "IAM", "SECURE DESIGN", "RESILIENCE", "NETWORK", "INCIDENT RESPONSE"].map((domain) => <span key={domain} className="rounded-lg border border-white/[.07] bg-white/[.025] px-2.5 py-2 font-mono text-[9px] text-slate-400">{domain}</span>)}</div><p className="mt-5 text-xs leading-5 text-slate-500">All facts, files, systems and scenarios are fictional. The terminal runs a sealed command set only.</p></div></div></section>;
}
...
export default function IsacaEvent() {
  ...
  const handleStart = () => {
    ...
    setAnswer("");
    start.mutate(undefined, { onSuccess: (result) => { loadResult(result); if (result.restarted) setNotice("Fresh sealed case opened. Restart from Stage 01; this run has a new server-generated evidence set."); if (result.outcome === "event_closed") setNotice("This event is permanently available."); }, onError: (error) => setNotice(error.message) });
  };
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    submitStage.mutate({ answer }, { onSuccess: (result) => {
      ...
      if (result.outcome === "time_expired") setNotice("The one-hour server timer has expired. This competition run is closed.");
      if (result.outcome === "event_closed") setNotice("This competition run is closed.");
  ...
  useEffect(() => {
    ...
    expire.mutate(undefined, { onSuccess: (result) => { if (result.run) setRun(result.run as Run); setNotice(result.outcome === "event_closed" ? "This competition run is closed." : "The one-hour server timer has expired. This competition run is closed."); } });
  }, [expire, run, timeRemaining]);

  const closeReason = status.data?.closed ? "This event is permanently available." : "";
