...
The ISACA Invitation route was checked at desktop (1280×720) and mobile (390×844). The event card is discoverable in Mission Control; the entry screen cleanly presents permanent availability, the one-hour run limit, three attempts per stage, no-hint rule, server verification, first-ten capacity, and the GRC/audit/risk domain coverage. The mobile layout keeps the mission briefing, event rules, and navigation readable without horizontal overflow.

The unauthenticated access guard was confirmed in the browser: the event presents a “Sign in to compete” control rather than starting or exposing any stage. Unit tests verify fifteen numbered seeded stages, unique cross-seed tags, server-only answers, response normalisation, the one-hour per-run deadline, and permanent event availability. The production build, TypeScript check, and all 11 unit tests pass.
